...
use fiscal_core::xml_builder::access_key::{calculate_mod11, format_year_month, generate_numeric_code};
...
pub fn build_mdfe_access_key(params: &MdfeAccessKeyParams<'_>) -> Result<MdfeAccessKey, FiscalError> {
    let base = format!(
...
mod tests {
    ...
    fn builds_44_digit_key_with_correct_layout() {
        ...
        assert_eq!(
            ak.key.as_bytes()[43] - b'0',
            calculate_mod11(&ak.key[..43])
        );
    }
